...
export default function ChatClient({ conversationId, initialMessages }: ChatClientProps) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "( >_< ) Hubo un error..." },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans overflow-hidden transition-colors pb-16 md:pb-0">
      <Sidebar currentPath="/" />
        ...
        <div className="p-4 bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-md">
          <div className="max-w-3xl mx-auto relative">
            <input
              ...
              disabled={loading}
            />
            <button onClick={sendMessage} disabled={loading} className="absolute right-2 top-1/2 -translate-y-1/2 bg-emerald-600 hover:bg-emerald-700 text-white p-2.5 rounded-full transition-all shadow-sm disabled:opacity-50">
