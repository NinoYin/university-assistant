...
import { signOut } from "next-auth/react"; // Importamos signOut para el cliente
        ...
        <aside className="w-72 bg-slate-50 dark:bg-slate-900/50 border-r border-slate-200 dark:border-slate-800 hidden md:flex flex-col transition-colors duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="space-y-1">
              {["Fechas de inscripción", "Requisitos para titulación", "Mapa curricular"].map((text) => (
                <button 
                  key={text}
          ...
          {/* Botón de cerrar sesión integrado en la barra lateral */}
          <div className="p-4 border-t border-slate-200 dark:border-slate-800">
            <button 
              onClick={() => signOut()} 
              className="w-full text-center text-sm font-medium text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400 transition-colors py-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30"
            >
              Cerrar sesión ( •_•)ﾉ
            </button>
          </div>
        ...
        <main className="flex-1 flex flex-col h-full relative">
          ...
          <header className="md:hidden flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-10">
            ...
            <div className="flex gap-2">
              ...
              <button 
                onClick={() => signOut()}
                className="text-xs font-medium bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full text-slate-600 dark:text-slate-300"
              >
                Salir
              </button>
            </div>
              ...
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full min-h-[50vh] text-slate-500 dark:text-slate-400 space-y-4 mt-10">
                  ...
                  <p className="text-center text-sm max-w-md">
                    Pregúntame sobre horarios, calificaciones, mapas curriculares o trámites.
                  </p>
                </div>
              )}
